...
docker_setup_db() {
  ...
  # exercise-promo: test db
  if [ -n "$MYSQL_DATABASE_TEST" ]; then
    mysql_note "Creating test database ${MYSQL_DATABASE_TEST}"
		docker_process_sql --database=mysql <<<"CREATE DATABASE IF NOT EXISTS \`$MYSQL_DATABASE_TEST\` ;"
  fi
    ...
    # exercise-promo: grant access to test db
		if [ -n "$MYSQL_DATABASE_TEST" ]; then
			mysql_note "Giving user ${MYSQL_USER} access to test schema ${MYSQL_DATABASE_TEST}"
			docker_process_sql --database=mysql <<<"GRANT ALL ON \`${MYSQL_DATABASE_TEST//_/\\_}\`.* TO '$MYSQL_USER'@'%' ;"
		fi
	fi
